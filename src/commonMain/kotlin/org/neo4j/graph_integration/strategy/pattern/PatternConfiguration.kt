...
import org.neo4j.graph_integration.utils.isJavaIdentifierStart
...
private fun isHomogeneousPattern(type: PatternConfigurationType, properties: List<String>, pattern: String, entityType: String) {
    val isHomogeneous = when (type) {
        PatternConfigurationType.INCLUDE -> properties.all { it.trim().isJavaIdentifierStart() }
...
data class NodePatternConfiguration(val keys: Set<String>, val type: PatternConfigurationType,
    ...
    companion object {
        ...
        // (:LabelA{!id,foo,bar})
        private val cypherNodePatternConfigured = """\((:\w+\s*(?::\s*(?:\w+)\s*)*)\s*(?:\{\s*(-?[\w!\.]+\s*(?:,\s*-?[!\w\*\.]+\s*)*)\})?\)$""".toRegex()
        // LabelA{!id,foo,bar}
        private val simpleNodePatternConfigured = """^(\w+\s*(?::\s*(?:\w+)\s*)*)\s*(?:\{\s*(-?[\w!\.]+\s*(?:,\s*-?[!\w\*\.]+\s*)*)\})?$""".toRegex()
...
data class RelationshipPatternConfiguration(val start: NodePatternConfiguration, val end: NodePatternConfiguration,
    ...
    fun isRelationshipProperty(propertyName: String): Boolean {
        return (!start.keys.contains(propertyName)
                && !start.properties.contains(propertyName)
                && !end.keys.contains(propertyName)
                && !end.properties.contains(propertyName))
    }
